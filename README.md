...
- `main.js` -Application entry point. Handles initial setup and shared orchestration across pages. Designed to scale as additional modules (e.g., creative logs) are imported.
- `site-menu.js` — Controls the global menu drawer behavior.
- `help.js` — Manages help card toggle and guidance interactions.
...
- `pages/creativeLogPage.js` -Handles layout and logic for the creative log page.
- `pages/recipesPage.js` - Manages structure and interactions for the recipes page.
...
- `word_old.js` - Controls word transitions and theme switching for the universe page.
- `gallery.js` —Implements lightbox interactions.
- `timer-base.js` — Core logic for the mini timer feature.
- `clock.js` — Decorative animated clock component.
- `hero.js` -Handles hero video playback behavior.

The project is deployed via GitHub Pages. The docs/ directory is kept up to date, and direct commits to the release branch are avoided in favor of working branches.
...
- All recipes images were generated by chatGPT.
...
### v0.1.9 (Recipes Launch) 2026 FEB
...
※この文章はchatGPTによって作成されたものです。文法や表現に間違いがあるかもしれませんが、そのまま掲載しています。
